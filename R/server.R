...
explorer_server <- function(input, output, session, data, verbose = FALSE) {
  ...
  dimension_reduction_df <- data.frame(
    Element = paste0(dimension_reduction_UI_names, ".UI"),
  ...
  assay_allowed_slots <- list(
    "FeatureAssay" = isolate(data$assay_slots),
  ...
  assay_df <- data.frame(
    Element = paste0(names(assay_allowed_slots), "s.UI"),
  ...
  output_assay <- lapply(1:nrow(assay_df), function(i) {
    output[[assay_df$Element[i]]] <- renderUI({
      if (verbose) {
        message(paste0("SeuratExplorer: preparing ", assay_df$Element[i], "..."))
      }
      assays_options <- filter_assay(
        assay_info = data$assays_slots_options,
  ...
  output$DimHighlightedClusters.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing DimHighlightedClusters.UI...")
    }
    shinyWidgets::pickerInput(
      inputId = "DimHighlightedClusters", label = "Highlight Clusters:",
  ...
  dimplot_width <- reactive({
    session$clientData$output_dimplot_width
  })
  ...
  output$dimplot <- renderPlot(
    {
      ...
      theme_choice <- input$DimPlotThemeStatic %||% "classic"
  ...
  output$FeatureAssaySlots.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing FeatureAssaySlots.UI...")
    }
    slot_choices <- filter_slot(
      assay_info = data$assays_slots_options,
  ...
  output$Featurehints.UI <- renderUI({
    ...
    helpText(
      strong(paste("Also supports: ",
        paste(data$extra_qc_options, collapse = " "), ".",
        sep = ""
      )),
      br(),
      strong("Tips: You can paste multiple genes from a column in excel."),
  ...
  observeEvent(input$FeatureGeneSymbol, {
    features_input <- CheckGene(
      InputGene = input$FeatureGeneSymbol,
      GeneLibrary = c(
        rownames(data$obj@assays[[input$FeatureAssay]]),
  ...
  featureplot_width <- reactive({
    session$clientData$output_featureplot_width
  })
  ...
  output$featureplot <- renderPlot(
    {
      ...
      if (any(is.na(features_dimplot$features_current))) { # when NA value
        ...
        if (!any(features_dimplot$features_current %in% c(rownames(cds[[input$FeatureAssay]]), data$extra_qc_options))) {
          ...
          color_scale <- input$FeaturePlotColorStaticScale %||% "default"
          if (color_scale == "default") {
            color_cols <- c(input$FeaturePlotLowestExprColor, input$FeaturePlotHighestExprColor)
          } else if (color_scale == "viridis") {
            color_cols <- viridis::viridis(100)
          } else if (color_scale == "plasma") {
            color_cols <- viridis::plasma(100)
          } else if (color_scale == "inferno") {
            color_cols <- viridis::inferno(100)
          } else if (color_scale == "magma") {
            color_cols <- viridis::magma(100)
          } else {
            color_cols <- c(input$FeaturePlotLowestExprColor, input$FeaturePlotHighestExprColor)
          }
          ...
          # Apply theme selection
          theme_choice <- input$FeaturePlotThemeStatic %||% "classic"
          if (theme_choice == "minimal") {
            p <- p + theme_minimal()
          } else if (theme_choice == "bw") {
            p <- p + theme_bw()
  ...
  output$featureplot_interactive <- plotly::renderPlotly({
    req(input$FeatureSlot)
    if (verbose) {
      message("SeuratExplorer: preparing featureplot_interactive...")
    }
    if (input$FeatureMinCutoff == 0) {
      expr_min_cutoff <- NA
    } else {
    ...
    if (any(is.na(features_dimplot$features_current))) { # when NA value
      ...
      if (!any(features_dimplot$features_current %in% c(rownames(cds[[input$FeatureAssay]]), data$extra_qc_options))) {
        ...
        # Create enhanced interactive feature plot
        coords <- Embeddings(cds, reduction = input$FeatureDimensionReduction)
        expr_data <- FetchData(cds, vars = features_dimplot$features_current, slot = input$FeatureSlot)

        width_px <- session$clientData$output_featureplot_width %||% 600
        height_px <- width_px * input$FeaturePlotHWRatio

        if (length(features_dimplot$features_current) == 1) {
          feature <- features_dimplot$features_current[1]

          # Create enhanced hover text based on selected tooltip variables
          tooltip_vars <- input$FeaturePlotTooltipVars %||% c("expression", "cluster")
          meta <- cds@meta.data

          hover_components <- list()
          if ("expression" %in% tooltip_vars) {
            hover_components[["Expression"]] <- round(expr_data[[feature]], 3)
          }
          if ("cluster" %in% tooltip_vars && input$FeatureClusterResolution %in% colnames(meta)) {
            hover_components[["Cluster"]] <- meta[[input$FeatureClusterResolution]]
          }
          if ("nFeature_RNA" %in% tooltip_vars && "nFeature_RNA" %in% colnames(meta)) {
            hover_components[["Features"]] <- meta$nFeature_RNA
          }
          if ("nCount_RNA" %in% tooltip_vars && "nCount_RNA" %in% colnames(meta)) {
            hover_components[["UMI Count"]] <- meta$nCount_RNA
          }

          hover_text <- paste(
            paste(names(hover_components), hover_components, sep = ": "),
            collapse = "<br>"
          )

          # Get color scale choice
          color_scale <- input$FeaturePlotColorScale %||% "viridis"
          ...
          plotly::plot_ly(
            ...
            type = "scatter",
            mode = "markers",
            marker = list(size = input$FeaturePointSize * 3, opacity = input$FeaturePointAlpha),
            colorscale = color_scale
          ) %>%
            plotly::layout(
              title = list(text = paste("Interactive Feature Plot:", feature)),
              xaxis = list(title = paste0(toupper(input$FeatureDimensionReduction), "_1")),
              yaxis = list(title = paste0(toupper(input$FeatureDimensionReduction), "_2")),
              width = width_px,
              height = height_px,
              template = input$FeaturePlotTheme %||% "plotly_white"
            ) %>%
            plotly::colorbar(title = "Expression")
        } else {
          # Multiple features - create subplots
          plot_list <- list()
          n_features <- length(features_dimplot$features_current)
          ncol_subplot <- ceiling(sqrt(n_features))
          nrow_subplot <- ceiling(n_features / ncol_subplot)

          color_scale <- input$FeaturePlotColorScale %||% "viridis"

          for (i in seq_along(features_dimplot$features_current)) {
            feature <- features_dimplot$features_current[i]
            ...
            p_sub <- plotly::plot_ly(
              ...
              type = "scatter",
              mode = "markers",
              marker = list(size = input$FeaturePointSize * 2),
              colorscale = color_scale,
              showscale = (i == 1)
            )
            plot_list[[i]] <- p_sub
          ...
          plotly::subplot(
            ...
            nrows = nrow_subplot,
            ncols = ncol_subplot,
            subplot_titles = features_dimplot$features_current
          ) %>%
            plotly::layout(
              title = list(text = "Interactive Feature Plot"),
              width = width_px,
              height = height_px * nrow_subplot * 0.8,
              template = input$FeaturePlotTheme %||% "plotly_white"
            )
        }
      }
  ...
  output$VlnAssaySlots.UI <- renderUI({
    req(input$VlnAssay)
    if (verbose) {
      message("SeuratExplorer: preparing VlnAssaySlots.UI...")
    }
    slot_choices <- filter_slot(
      assay_info = data$assays_slots_options,
  ...
  observeEvent(input$VlnGeneSymbol, {
    features_input <- CheckGene(
      InputGene = input$VlnGeneSymbol,
      GeneLibrary = c(
        rownames(data$obj@assays[[input$VlnAssay]]),
        data$extra_qc_options
      )
    )
  ...
  output$VlnIdentsSelected.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "VlnIdentsSelected", label = "Clusters Used:",
  ...
  vlnplot_width <- reactive({
    session$clientData$output_vlnplot_width
  })
    ...
    if (any(is.na(features_vlnplot$features_current))) {
      ...
      if (!any(features_vlnplot$features_current %in% c(rownames(cds[[input$VlnAssay]]), data$extra_qc_options))) {
        ...
        if (length(features_vlnplot$features_current) == 1) {
          ...
          # Multiple genes - different handling for separate conditions
          if (!separate_conditions) {
            # For multiple genes, create base plot (will be handled by interactive function)
            p <- Seurat::VlnPlot(cds,
  ...
  output$DotIdentsSelected.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "DotIdentsSelected", label = "Clusters Used:",
  ...
  dotplot_width <- reactive({
    session$clientData$output_dotplot_width
  })
  ...
  output$dotplot <- renderPlot(
    {
      ...
      if (any(is.na(features_dotplot$features_current))) { # NA
        ...
        if (!any(features_dotplot$features_current %in% rownames(cds[[input$DotAssay]]))) {
          ...
          p <- p & ggplot2::theme(
            axis.text.x = ggplot2::element_text(size = input$DotXlabelSize),
            axis.text.y = ggplot2::element_text(size = input$DotYlabelSize)
          )
          if (input$DotRotateAxis) {
            p <- p + Seurat::RotatedAxis()
          }
          if (input$DotFlipCoordinate) {
  ...
  output$dotplot_interactive <- plotly::renderPlotly({
    if (verbose) {
      message("SeuratExplorer: preparing dotplot_interactive...")
    }
    if (any(is.na(features_dotplot$features_current))) { # NA
  ...
  output$HeatmapAssaySlots.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing HeatmapAssaySlots.UI...")
    }
    slot_choices <- filter_slot(
      assay_info = data$assays_slots_options,
  ...
  observeEvent(input$HeatmapGeneSymbol, {
    features_input <- CheckGene(
      InputGene = input$HeatmapGeneSymbol,
  ...
  heatmap_width <- reactive({
    session$clientData$output_heatmap_width
  })
  ...
  output$heatmap <- renderPlot(
    {
      ...
      if (any(is.na(features_heatmap$features_current))) { # NA
        ...
        if (!any(features_heatmap$features_current %in% rownames(cds[[input$HeatmapAssay]]))) {
          ...
          # Apply color palette
          palette_choice <- input$HeatmapColorPalette %||% "default"
          if (palette_choice == "viridis") {
            ...
            p <- p + ggplot2::scale_fill_viridis_c(option = "cividis")
          } else if (palette_choice == "RdYlBu") {
            p <- p + ggplot2::scale_fill_distiller(palette = "RdYlBu")
          } else if (palette_choice == "RdBu") {
            p <- p + ggplot2::scale_fill_distiller(palette = "RdBu")
          } else if (palette_choice == "Spectral") {
  ...
  # Interactive Heatmap output using plotly
  output$heatmap_interactive <- plotly::renderPlotly({
    if (verbose) {
      message("SeuratExplorer: preparing heatmap_interactive...")
    }
    if (any(is.na(features_heatmap$features_current))) { # NA
      interactive_empty_plot() # when no symbol or wrong input, show a blank pic.
    } else {
      ...
      if (!any(features_heatmap$features_current %in% rownames(cds[[input$HeatmapAssay]]))) {
        ...
        Seurat::Idents(cds) <- input$HeatmapClusterResolution
        Seurat::DefaultAssay(cds) <- input$HeatmapAssay

        # Get the expression data
        if (input$HeatmapSlot == "scale.data") {
          expr_data <- GetAssayData(cds, slot = "scale.data", assay = input$HeatmapAssay)[features_heatmap$features_current, , drop = FALSE]
        } else {
          expr_data <- GetAssayData(cds, slot = input$HeatmapSlot, assay = input$HeatmapAssay)[features_heatmap$features_current, , drop = FALSE]
        }

        # Order cells by cluster
        cell_order <- order(cds@meta.data[[input$HeatmapClusterResolution]])
        expr_data <- expr_data[, cell_order, drop = FALSE]
        cluster_info <- cds@meta.data[cell_order, input$HeatmapClusterResolution]
        ...
        if (palette_choice == "viridis") {
          ...
          colorscale <- "RdBu" # default
        }

        # Create hover text
        hover_text <- matrix(nrow = nrow(expr_matrix), ncol = ncol(expr_matrix))
        for (i in 1:nrow(expr_matrix)) {
          for (j in 1:ncol(expr_matrix)) {
            hover_text[i, j] <- paste0(
              "Gene: ", rownames(expr_matrix)[i], "<br>",
              "Cell: ", colnames(expr_matrix)[j], "<br>",
              "Cluster: ", cluster_info[j], "<br>",
              "Expression: ", round(expr_matrix[i, j], 3)
            )
          }
        }

        width_px <- session$clientData$output_heatmap_width %||% 600
        height_px <- width_px * input$HeatmapPlotHWRatio
        ...
        p <- plotly::plot_ly(
          ...
          plotly::layout(
            title = list(text = "Interactive Gene Expression Heatmap", font = list(size = 16)),
            xaxis = list(
              title = "Cells",
              showticklabels = if (input$HeatmapShowColumnNames %||% TRUE) TRUE else FALSE,
              tickangle = 45
            ),
            yaxis = list(
              title = "Genes",
              showticklabels = if (input$HeatmapShowRowNames %||% TRUE) TRUE else FALSE
            ),
            width = width_px,
            height = height_px
          ) %>%
  ...
  observeEvent(input$AveragedHeatmapGeneSymbol, {
    features_input <- CheckGene(
      InputGene = input$AveragedHeatmapGeneSymbol,
  ...
  output$AveragedHeatmapClusterOrder.UI <- renderUI({
    ...
    shinyjqui::orderInput(
      inputId = "AveragedHeatmapClusterOrder",
      label = "Drag to order:",
  ...
  output$AveragedHeatmapIdentsSelected.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "AveragedHeatmapIdentsSelected", label = "Clusters Used:",
  ...
  averagedheatmap_width <- reactive({
    session$clientData$output_averagedheatmap_width
  })
  ...
  output$averagedheatmap <- renderPlot(
    {
      ...
      if (any(is.na(features_heatmap_averaged$features_current))) { # NA
        ...
        if (!any(features_heatmap_averaged$features_current %in% rownames(cds[[input$AveragedHeatmapAssay]]))) {
          ...
          p <- suppressMessages(AverageHeatmap(
            object = cds,
      ...
      pdf(
        file = paste0(temp_dir, "/AveragedHeatmap.pdf"),
  ...
  output$RidgeplotAssaySlots.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing RidgeplotAssaySlots.UI...")
    }
    slot_choices <- filter_slot(
      assay_info = data$assays_slots_options,
  ...
  observeEvent(input$RidgeplotGeneSymbol, {
    features_input <- CheckGene(
      InputGene = input$RidgeplotGeneSymbol,
      GeneLibrary = c(
        rownames(data$obj@assays[[input$RidgeplotAssay]]),
        data$extra_qc_options
      )
    )
  ...
  output$Ridgeplothints.UI <- renderUI({
    ...
    helpText(
      strong(paste("Also supports: ", paste(data$extra_qc_options, collapse = " "),
        ".",
        sep = ""
      )),
      br(),
  ...
  output$RidgeplotIdentsSelected.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "RidgeplotIdentsSelected", label = "Clusters Used:",
  ...
  ridgeplot_width <- reactive({
    session$clientData$output_ridgeplot_width
  })
      ...
      if (any(is.na(features_ridgeplot$features_current))) { # NA
        ...
        if (!any(features_ridgeplot$features_current %in% c(rownames(cds[[input$RidgeplotAssay]]), data$extra_qc_options))) {
          ...
          p <- Seurat::RidgePlot(
            ...
            ggplot2::theme(
              axis.text.x = ggplot2::element_text(size = input$RidgeplotXlabelSize),
  ...
  output$CellratioplotXOrder.UI <- renderUI({
    ...
    shinyjqui::orderInput(
      inputId = "CellratioXOrder", label = "Drag to order:",
  ...
  cellratioplot_width <- reactive({
    session$clientData$output_cellratioplot_width
  })
  ...
  output$IntraClusterDEGsCustomizedGroupsControl.UI <- renderUI({
    ...
    selectInput("IntraClusterDEGsCustomizedGroupsControl", "Choose control groups:",
      multiple = TRUE,
      choices = setdiff(
        levels(data$obj@meta.data[, input$IntraClusterDEGsCustomizedGroups]),
  ...
  output$IntraClusterDEGsSubsetCellsSelectedClusters.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "IntraClusterDEGsSubsetCellsSelectedClusters", label = "Cells to Keep:",
  ...
  observeEvent(input$IntraClusterDEGssAnalysis, {
    ...
    if (any(
      is.null(input$IntraClusterDEGsCustomizedGroupsCase),
      ...
      showModal(modalDialog(
        title = "Calculating DEGs...", "Please wait for a few minutes!",
        footer = NULL,
        size = "l"
      ))
      cds <- data$obj
  ...
  # Access GenesDB dataset from package data
  data("GenesDB", package = "SeuratExplorer", envir = environment())
  db <- GenesDB
  ...
  output$TopGenesSelectedClusters.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing TopGenesSelectedClusters.UI...")
    }
    shinyWidgets::pickerInput(
      inputId = "TopGenesSelectedClusters",
      label = "Subset cells:",
  ...
  observeEvent(input$TopAccumulatedGenesAnalysis, {
    ...
    cds <- subset_Seurat(cds, idents = input$TopGenesSelectedClusters)
    if (input$TopGenesClusterLevel) {
      TopGenes$topgenes <- top_accumulated_genes(
        SeuratObj = cds,
    ...
    if (nrow(TopGenes$topgenes) > 0) {
      ...
      showModal(modalDialog(
        title = "Error", "No genes found, please check the parameters.",
        footer = modalButton("Dismiss"),
        easyClose = TRUE,
        size = "l"
      ))
  ...
  output$dataset_topgenes <- DT::renderDT(server = FALSE, {
    ...
    # Show data
    DT::datatable(TopGenes$topgenes,
      extensions = "Buttons",
      options = list(
        scrollX = TRUE,
  ...
  output$FeatureSummarySelectedClusters.UI <- renderUI({
    ...
    if (verbose) {
      message("SeuratExplorer: preparing FeatureSummarySelectedClusters.UI...")
    }
    shinyWidgets::pickerInput(
      inputId = "FeatureSummarySelectedClusters", label = "Subset cells:",
  ...
  observeEvent(input$FeatureSummaryAnalysis, {
    ...
    if (is.na(input$FeatureSummarySymbol)) {
      ...
      GeneRevised <- CheckGene(
        InputGene = input$FeatureSummarySymbol,
    ...
    if (any(is.na(GeneRevised))) {
      ...
      if (input$FeatureSummaryClusterLevel) {
        FeatureSummary$summary <- summary_features(
          SeuratObj = cds,
  ...
  output$dataset_featuresummary <- DT::renderDT(server = FALSE, {
    ...
    # Show data
    DT::datatable(FeatureSummary$summary,
      extensions = "Buttons",
      options = list(
        scrollX = TRUE,
  ...
  output$FeatureCorrelationIdentsSelected.UI <- renderUI({
    ...
    shinyWidgets::pickerInput(
      inputId = "FeatureCorrelationIdentsSelected", label = "Clusters Used:",
  ...
  observeEvent(input$TopCorrelationAnalysis, {
    ...
    FeatureCorrelation$summary <- calculate_top_correlations(
      SeuratObj = cds,
  ...
  observeEvent(input$MostCorrelatedAnalysis, {
    ...
    feature.revised <- ReviseGene(
      Agene = trimws(input$MostCorrelatedAGene),
    ...
    if (is.na(feature.revised)) {
      ...
      Seurat::Idents(cds) <- input$FeatureCorrelationClusterResolution
      cds <- subset_Seurat(cds, idents = input$FeatureCorrelationIdentsSelected)
      FeatureCorrelation$summary <- calculate_most_correlated(
        SeuratObj = cds,
  ...
  observeEvent(input$calculatecorrelation, {
    ...
    if (is.na(input$CorrelationGeneList)) {
      ...
      GeneRevised <- CheckGene(
        InputGene = input$CorrelationGeneList,
        GeneLibrary = rownames(data$obj[[input$FeatureCorrelationAssay]])
      )
    }
    if (any(is.na(GeneRevised))) {
      ...
      Seurat::Idents(cds) <- input$FeatureCorrelationClusterResolution
      cds <- subset_Seurat(cds, idents = input$FeatureCorrelationIdentsSelected)
      FeatureCorrelation$summary <- calculate_correlation(
        SeuratObj = cds,
        features = GeneRevised,
        method = input$correlationmethod,
        assay = input$FeatureCorrelationAssay
  ...
  output$dataset_correlation <- DT::renderDT(server = FALSE, {
    ...
    # Show data
    DT::datatable(FeatureCorrelation$summary,
      extensions = "Buttons",
      options = list(
        scrollX = TRUE,
        paging = TRUE, searching = TRUE,
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    validate(need(
      expr = ext %in% c("rds", "qs2", "Rds"),
      message = "Please upload a .rds or a .qs2 file"
    ))
    data$obj <- prepare_seurat_object(
      obj = readSeurat(path = input$dataset_file$datapath),
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$reduction_options <- prepare_reduction_options(
      obj = data$obj,
      keywords = getOption("SeuratExplorerReductionKeyWords"),
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$assays_slots_options <- prepare_assays_slots(
      ob = data$obj,
      data_slot = data$assay_slots,
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$assays_options <- prepare_assays_options(
      Alist = data$assays_slots_options,
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$assay_default <- ifelse(data$assay_default %in% data$assays_options, data$assay_default,
      data$assays_options[1]
    ) # update the default assay

    data$cluster_options <- prepare_cluster_options(
      df = data$obj@meta.data,
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$gene_annotions_list <- prepare_gene_annotations(
      obj = data$obj,
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$split_options <- prepare_split_options(
      df = data$obj@meta.data,
      max.level = data$split_maxlevel,
      verbose = getOption("SeuratExplorerVerbose")
    )

    data$extra_qc_options <- prepare_qc_options(
      df = data$obj@meta.data,
